...
func NewAnnounce(req *http.Request) (*Announce, []string) {
	req.ParseForm()
	data := req.Form
	errors := GetAnnounceErrors(data)

	fmt.Printf("request values: \n")
	...
	announce.InfoHash = []byte(data.Get("info_hash"))
	announce.PeerId = []byte(data.Get("peer_id"))
